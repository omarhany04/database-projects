...
<body class="bg-gray-800 p-8">
  ...
  <h1 class="text-3xl font-bold mb-6 text-blue-400">Members</h1>
  ...
  <form action="add_record.php" method="POST" class="mb-8 flex flex-wrap gap-4 p-4 bg-gray-700 rounded-lg shadow-inner">
    ...
    <input name="name" placeholder="Name" class="form-input-styled w-full md:w-1/4">
    <input name="address" placeholder="Address" class="form-input-styled w-full md:w-1/4">
    <input name="join_date" type="date" class="form-input-styled w-full md:w-1/6">
  ...
  <table class="w-full custom-table bg-gray-700 rounded-xl shadow-2xl">
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        echo "<td class='p-4'>{$row['join_date']}</td>";
        echo "<td class='p-4'><a href='delete_record.php?table=member&id={$row['member_id']}' class='text-red-600 hover:underline'>Delete</a></td>";
        echo "</tr>";
    }
    ?>
  </table>
