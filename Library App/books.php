...
<body class="bg-gray-800 p-8">
  ...
  <h1 class="text-3xl font-bold mb-6 text-blue-400">Books</h1>
  ...
  <form action="add_record.php" method="POST" class="mb-8 flex flex-wrap gap-4 p-4 bg-gray-700 rounded-lg shadow-inner">
    ...
    <input name="title" placeholder="Title" class="form-input-styled w-full md:w-1/4">
    <input name="price" placeholder="Price" class="form-input-styled w-full md:w-1/6">
    <input name="pub_id" placeholder="Publisher ID" class="form-input-styled w-full md:w-1/6">
    <input name="category_id" placeholder="Category ID" class="form-input-styled w-full md:w-1/6">
  ...
  <table class="w-full custom-table bg-gray-700 rounded-xl shadow-2xl">
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        echo "<td class='p-4'>{$row['price']}</td>";
        echo "<td class='p-4'>{$row['pub_id']}</td>";
        echo "<td class='p-4'>{$row['category_id']}</td>";
        echo "<td class='p-4'><a href='delete_record.php?table=book&id={$row['book_id']}' class='text-red-600 hover:underline'>Delete</a></td>";
        echo "</tr>";
    }
    ?>
  </table>
